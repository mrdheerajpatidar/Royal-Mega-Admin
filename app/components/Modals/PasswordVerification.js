...
import { agentBalance } from '@utils/Endpoints';
...
const PasswordVerification = ({ isOpen, closeModal, updater, type }) => {
  ...
  const addChipsHandler = async (e) => {
    ...
    try {
      if (valid) {
        setIsLoading(true);
        const res = await agentBalance(payload);
        const { status, error } = res;
        if (status) {
          toast.success('Balance Added Successfully', { toastId: 1 });
        } else {
